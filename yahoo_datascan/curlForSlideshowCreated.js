...
//var japi_host="japi1.global.media.yahoo.com";
    ...
    function getIndivSlideshow(status, data){        
        ...
        for(var id in data.hits){
            ...
            curl(japi_host, port, endpoint, "GET", checkSlideshowCreated, uuid);
        }
    }

    function checkSlideshowCreated(status, data, uuid){
        if (200!=status){
        ...
        current_count++;
	var ss=data.content["yahoo-media:slideshow"];
	if (typeof ss["created"]!="undefined" && ss["created"].length!=0){
            console.log(current_count+">> UUID "+uuid+" contains created, downloading...."); 
            fs.writeFile(target_dir+"/"+uuid+".json", JSON.stringify(data.content["yahoo-media:slideshow"], null, 4), function(err) {
                if(err) {
